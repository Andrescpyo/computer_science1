...
async function loadSongs() {
    try {
        ...
        // Asegurarse de que nextId sea mayor que el ID más alto del JSON
        const maxId = songsData.reduce((max, song) => Math.max(max, song.id), 0);
        musicLibrary.nextId = maxId + 1;
        ...
        libraryGrid.innerHTML = '<p style="color: var(--text-color-dark); text-align: center; width: 100%;">Error al cargar las canciones. Asegúrate de que el archivo `songs.json` existe y estás ejecutando un servidor local.</p>';
    }
}
 ...
 * excede su ancho visible. Ahora duplica el texto para un scroll continuo.
 * @param {HTMLElement} element El elemento HTML (p.ej., <p class="album-name">) que contiene el texto.
 */
function applyMarqueeEffect(element) {
    // Primero, limpiar cualquier contenido y animación anterior para evitar duplicados
    element.innerHTML = '';
    element.style.animationDuration = ''; // Limpiar duración de animación anterior
    element.classList.remove('marquee');  // Remover clase marquee si estaba aplicada

    const originalText = element.getAttribute('title') || element.textContent; // Usar el title o textContent
    const span = document.createElement('span');
    span.classList.add('marquee-text');
    span.textContent = originalText;
    element.appendChild(span); // Añadir la primera copia del texto

    // Esperar un poco para que el navegador renderice el texto y podamos medir su ancho.
    // Esto es crucial para mediciones precisas del scrollWidth.
    setTimeout(() => {
        // Calcular si el texto original desborda el contenedor
        // El 'span' contiene el texto y tendrá el 'scrollWidth' real
        const textWidth = span.scrollWidth;
        const containerWidth = element.clientWidth; // El ancho visible del <p>

        if (textWidth > containerWidth) {
            // El texto desborda, duplicarlo y aplicar marquesina
            const duplicatedTextSpan = document.createElement('span');
            duplicatedTextSpan.classList.add('marquee-text'); // Misma clase para animar
            duplicatedTextSpan.textContent = originalText;
            element.appendChild(duplicatedTextSpan); // Añadir la segunda copia

            element.classList.add('marquee'); // Añadir la clase para activar la animación CSS

            // Calcular la duración de la animación basada en la longitud del texto
            // Esto asegura que textos más largos se desplacen más rápido para completar el ciclo
            // Ajusta el 'speedFactor' (p.ej., 50) para controlar la velocidad general.
            // A mayor speedFactor, más lenta la animación.
            const speedFactor = 40; // Pixeles por segundo aproximadamente
            const animationDuration = textWidth / speedFactor; // Duración en segundos

            // Ajustar la duración de la animación en el CSS
            // Multiplicamos por 2 porque la animación mueve el 50% del total (texto duplicado)
            element.style.animationDuration = `${animationDuration * 2}s`;

            // Para pausar al hover (opcional, si quieres que se mueva solo al pasar el ratón)
            /*
            ...
            */
            ...
            // Si el texto no desborda, asegúrate de que no tenga la clase 'marquee'
            element.classList.remove('marquee');
            element.style.animationDuration = ''; // Limpiar la duración de la animación
            element.innerHTML = originalText; // Restaurar solo el texto original sin spans adicionales
        }
    }, 50); // Pequeño retraso para que el DOM se asiente
}
...
/**
 * Renderiza los álbumes de la biblioteca en la cuadrícula principal.
 * Agrupa las canciones por álbum y artista para mostrar una tarjeta por álbum.
 */
function renderLibrarySongs() {
    ...
    // Paso 1: Agrupar canciones por álbum y artista
    const albums = new Map(); // Usaremos un Map para almacenar álbumes únicos
                               // Key: `${albumName}|${artistName}`
                               // Value: { album: string, artist: string, songs: Array<Song>, firstSong: Song }
    ...
    allSongs.forEach(song => {
        ...
        if (!albums.has(albumKey)) {
            albums.set(albumKey, {
                ...
                artist: song.artist,
                songs: [],
                firstSong: song // Guardamos la primera canción encontrada para este álbum
                                // Esto nos servirá para pasar un 'selectedSong' a showPlaylistScreen
            });
        }
        albums.get(albumKey).songs.push(song);
    });

    // Convertir el Map a un array para poder ordenarlo si es necesario
    const albumList = Array.from(albums.values());

    // Opcional: Ordenar los álbumes por nombre del álbum o artista
    albumList.sort((a, b) => a.album.localeCompare(b.album) || a.artist.localeCompare(b.artist));


    // Paso 2: Renderizar una tarjeta por cada álbum único
    albumList.forEach(albumData => {
        const card = document.createElement('div');
        card.classList.add('music-card');
        // Usar la clave del álbum para identificar la tarjeta si se necesita
        card.dataset.albumKey = `${albumData.album}|${albumData.artist}`;

        card.innerHTML = `
            <div class="album-cover" style="background: ${getRandomGradient()};"></div>
            <p class="album-name" title="${albumData.album}">${albumData.album}</p>
            <p class="artist">${albumData.artist}</p>
            <button class="delete-button" data-album-key="${albumData.album}|${albumData.artist}"><span class="fas fa-trash"></span></button>
        `;
        libraryGrid.appendChild(card);

        // Aplicar marquesina a los nombres de los álbumes
        const albumNameElement = card.querySelector('.album-name');
        applyMarqueeEffect(albumNameElement);

        // Eventos para ver detalles del álbum (que llevará a la pantalla de playlist)
        // Al hacer clic, pasaremos la 'firstSong' del álbum para que showPlaylistScreen
        // pueda identificar el álbum y cargar todas sus canciones.
        card.querySelector('.album-cover').addEventListener('click', () => showPlaylistScreen(albumData.firstSong));
        card.querySelector('.album-name').addEventListener('click', () => showPlaylistScreen(albumData.firstSong));
        card.querySelector('.artist').addEventListener('click', () => showPlaylistScreen(albumData.firstSong));

        // Evento para eliminar álbum (eliminar TODAS las canciones de ese álbum)
        card.querySelector('.delete-button').addEventListener('click', (event) => {
            event.stopPropagation();
            const albumKeyToDelete = event.currentTarget.dataset.albumKey;
            const albumInfo = albums.get(albumKeyToDelete);

            if (confirm(`¿Estás seguro de que quieres eliminar TODAS las canciones del álbum "${albumInfo.album}" de "${albumInfo.artist}"?`)) {
                let deletedCount = 0;
                albumInfo.songs.forEach(song => {
                    if (musicLibrary.removeSong(song.id)) {
                        deletedCount++;
                    }
                });

                if (deletedCount > 0) {
                    alert(`Se eliminaron ${deletedCount} canciones del álbum "${albumInfo.album}".`);
                    renderLibrarySongs(); // Volver a renderizar la biblioteca
                    ...
                    alert('No se pudieron eliminar las canciones del álbum.');
                }
            }
        });
    });
...
function renderSearchResults(results) {
    ...
    results.forEach(song => {
        ...
        // Aplicar marquesina a los títulos de los álbumes en los resultados (aquí son títulos de canciones)
